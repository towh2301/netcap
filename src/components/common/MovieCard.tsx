	...
	return (
		...
		>
			...
			{typeof rating === "number" && rating > 0 && (
				<div className="absolute top-2 right-2 z-10 px-2 py-0.5 rounded bg-yellow-500/90 text-black text-[10px] font-semibold">
					★ {rating.toFixed(1)}
				</div>
			)}
			...
			<div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
				<div className="size-10 rounded-full bg-black/60 backdrop-blur-sm flex items-center justify-center text-white">
			...
			>
				...
				<p className="text-[11px] text-gray-300 flex items-center gap-1">
					...
					{movie?.episode_current && (
						<span className="ml-2 px-1.5 py-0.5 rounded bg-white/10 text-white/90 border border-white/10">
							{movie.episode_current}
						</span>
					)}
				</p>
